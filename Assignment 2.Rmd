...
the growth rate of technology in steady state is effected by $\beta$
(elasticity of capital on knowledge??????). - $\gamma$ is the output
elasticity of labour allocated in R&D which we can see has a positive
effect on the growth rate. - $\theta$ is a parameter describing the
elasticity of existing knowledge (A), we can see this also has a
positive effect on $g^*_A$

Beta er elasticiteten på output når der allokeres 1 mere enhed af
